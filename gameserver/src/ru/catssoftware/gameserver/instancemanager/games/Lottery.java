...
public class Lottery
{
	...
	private class startLottery implements Runnable
	{
		...
		private void announceLottery()
		{
			...
			Announcements.getInstance().announceToAll("В продажу поступили лоттерейные билеты на " + getId() + "-й тур Лоттереи Удачи.");
		}
		...
		private void scheduleEndOfLottery()
		{
			...
			finishtime.add(Calendar.DAY_OF_MONTH, 2);
			finishtime.set(Calendar.DAY_OF_WEEK, 7);
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			if (_log.isDebugEnabled() || Config.DEBUG)
				_log.debug("Lottery: The lucky numbers are " + luckynums[0] + ", " + luckynums[1] + ", " + luckynums[2] + ", " + luckynums[3] + ", "
						+ luckynums[4] + ".");
