...
import ru.catssoftware.gameserver.model.SpawnListener;
...
public class SevenSignsFestival implements SpawnListener
{
	...
	// ////////////////////// \\\\\\\\\\\\\\\\\\\\\\\\\\
	/*
	 * The following contains all the necessary spawn data for: - Player Start Locations - Witches - Monsters - Chests
	 *
	 * All data is given by: X, Y, Z (coords), Heading, NPC ID (if necessary) This may be moved externally in time, but the data should not change.
	 */
	 ...
	 * Signs cycle, with _signsCycle representing data for the current round of Festivals. The actual table data is stored as a series of StatsSet constructs.
	...
	public static final String getFestivalNameText(int festivalID)
	{
		...
		{
		case FESTIVAL_LEVEL_MAX_31:
			festivalName = "31-й уровень и ниже";
			break;
		case FESTIVAL_LEVEL_MAX_42:
			festivalName = "42-й уровень и ниже";
			break;
		case FESTIVAL_LEVEL_MAX_53:
			festivalName = "53-й уровень и ниже";
			break;
		case FESTIVAL_LEVEL_MAX_64:
			festivalName = "64-й уровень и ниже";
			break;
		default:
			festivalName = "Без ограничения уровня";
			break;
		}
	...
	protected void restoreFestivalData()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (_log.isDebugEnabled() || Config.DEBUG)
					_log.debug("SevenSignsFestival: Loaded data from DB for (Cycle = " + festivalCycle + ", Oracle = " + cabal + ", Festival = "
	...
	/**
	 * Stores current festival data, basic settings to the properties file and past high score data to the database. If updateSettings = true, then all Seven
	 * Signs data is updated in the database.
	 *
	...
	public void saveFestivalData(boolean updateSettings)
	{
		...
		PreparedStatement statement = null;
		if(!updateSettings)
			System.out.print("Saving SevenSIgns data...");
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection(con);
	...
	{
		if (SevenSigns.getInstance().isSealValidationPeriod())
			return "<font color=\"FF0000\">В данный момент действует период владения печатью. Фестиваль тьмы продолжится на следующей неделе.</font>";

		int minToStart=getMinsToNextFestival();
		if (minToStart<2)
			return "<font color=\"FF0000\">Фестиваль тьмы начнется через " + minToStart + " митуту.</font>";
		if (minToStart<5)
			return "<font color=\"FF0000\">Фестиваль тьмы начнется через " + minToStart + " митуты.</font>";
		return "<font color=\"FF0000\">Фестиваль тьмы начнется через " + minToStart + " митут.</font>";
	}

	/**
	 * Returns the current festival ID and oracle ID that the specified player is in, but will return the default of {-1, -1} if the player is not found as a
	 * participant.
	 *
	 * @param player
	...
	public void setParticipants(int oracle, int festivalId, L2Party festivalParty)
	{
		...
		if (festivalParty != null)
		{
			...
			if (_log.isDebugEnabled() || Config.DEBUG)
				_log.debug("SevenSignsFestival: " + festivalParty.getPartyMembers().toString() + " have signed up to the "
	...
	/**
	 * Set the final score details for the last participants of the specified festival data. Returns <b>true</b> if the score is higher than that previously
	 * recorded <b>this cycle</b>.
	 *
	 * @param player
	 * @param oracle
	 * @param festivalId
	...
	/**
	 * Calculate and return the proportion of the accumulated bonus for the festival where the player was in the winning party, if the winning party's cabal won
	 * the event. The accumulated bonus is then updated, with the player's share deducted.
	 *
	 * @param player
	...
	public void sendMessageToAll(String senderName, String message)
	{
		...
		CreatureSay cs = new CreatureSay(_dawnChatGuide.getObjectId(), SystemChatChannelId.Chat_Shout, senderName, message);
		_dawnChatGuide.broadcastPacket(cs);

		cs = new CreatureSay(_duskChatGuide.getObjectId(), SystemChatChannelId.Chat_Shout, senderName, message);
	...
	private class FestivalManager implements Runnable
	{
		...
		public void run()
		{
			...
			sendMessageToAll("Гид Фестиваля", "Основной эвент начнется через " + getMinsToNextFestival() + " минут. Вы можете зарегистроваться на участие.");
			...
			sendMessageToAll("Гид Фестиваля", "Начинается основной эвент.");
			...
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants("Фестиваль Тьмы начался!");
			}
			...
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants("Фестиваль окончится через " + ((Config.ALT_FESTIVAL_LENGTH - Config.ALT_FESTIVAL_SECOND_SPAWN) / 60000) + " минут.");
			}
			...
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
			{
				...
				festivalInst.sendMessageToParticipants("Появились сундуки сокровищ фестиваля! Поспешите, фестиваль скоро закончится.");
			}
			...
			// delete all spawned mobs after the this cycle
			for (L2DarknessFestival festivalInst : _festivalInstances.values())
				festivalInst.unspawnMobs();

			sendMessageToAll("Ведьма фестиваля", "Ничего немогу поделать, но я отправляю вас обратно!");
	...
	/**
	 * Each running festival is represented by an L2DarknessFestival class. It contains all the spawn information and data for the running festival. All
	 * festivals are managed by the FestivalManager class, which must be initialized first.
	 *
	 * @author Tempy
	 */
	private class L2DarknessFestival
	{
		...
		protected void festivalInit()
		{
			...
			try
			{
				...
				if (_log.isDebugEnabled() || Config.DEBUG)
					_log.debug("SevenSignsFestival: Spawned the Festival Witch " + npcSpawn.getNpcId() + " at " + _witchSpawn._x + " " + _witchSpawn._y + " "
							+ _witchSpawn._z);
			}
			catch (Exception e)
			{
			...
			MagicSkillUse msu = new MagicSkillUse(_witchInst, _witchInst, 2003, 1, 1, 0, false);
			_witchInst.broadcastPacket(msu);

			// And another one...:D
			msu = new MagicSkillUse(_witchInst, _witchInst, 2133, 1, 1, 0, false);
			_witchInst.broadcastPacket(msu);

			// Send a message to all participants from the witch.
			sendMessageToParticipants("Фестиваль начнется через 2 минуты.");
		}
		...
		protected void moveMonstersToCenter()
		{
			...
			for (L2FestivalMonsterInstance festivalMob : _npcInsts)
			{
				...
				/*
				 * Random X and Y coords around the player start location, up to half of the maximum allowed offset are generated to prevent the mobs from all
		...
		{
			if (_log.isDebugEnabled() || Config.DEBUG)
				_log.debug("SevenSignsFestival: Modifying spawn rate of festival mobs to " + respawnDelay + " ms for festival " + SevenSigns.getCabalShortName(_cabal) + " (" + getFestivalName(_levelRange) + ")");
		...
		public void sendMessageToParticipants(String message)
		{
			if (!_participants.isEmpty())
			{
				CreatureSay cs = new CreatureSay(_witchInst.getObjectId(), SystemChatChannelId.Chat_Normal, "Ведьма Фестиваля", message);
			...
			if (!_participants.isEmpty())
			{
				for (L2PcInstance participant : _participants)
				{
					if (participant != null)
					{
						...
						participant.sendMessage("Фестиваль Тьмы окончен. Лидер вашей группы должен зарегистрировать результат до того, как стартует следующий эвент.");
					}
				}
		...
		public void relocatePlayer(L2PcInstance participant, boolean isRemoving)
		{
			try
			{
				...
				participant.sendMessage("Вы были удалены с арены Фестиваля.");
			}
			catch (Exception e)
			{
