...
public class SevenSigns
{
	...
	public static final String						SEVEN_SIGNS_DATA_FILE		= "./config/main/events/seven_signs.properties";
	...
	private static int[]							ANGELS_ID = {21187,21188,21189,21190,21191,21192,21193,21194,21195,21196,21197,21198,21199,21200,21201,21202,21203,21204,21205,21206,21207};
	private static int[]							DEMONS_ID = {21166,21167,21168,21169,21170,21171,21172,21173,21174,21175,21176,21177,21178,21179,21180,21181,21182,21183,21184,21185,21186};
	public static FastList<L2Spawn>					ANGELS = new FastList<L2Spawn>();
	public static FastList<L2Spawn>					DEMONS = new FastList<L2Spawn>();
	...
	protected int[]									_dawnFestivalIdScore = {0,0,0,0,0};
	protected int[]									_duskFestivalIdScore = {0,0,0,0,0};
	...
	private static ScheduledFuture<?>				_periodTimer;	
	...
	public static final String getCabalNameText(int cabal)
	{
		...
		{
		case CABAL_DAWN:
			return "Лорды Рассвета";
		case CABAL_DUSK:
			return "Революционеры Заката";
		}
	...
	{
		String sealName = (!shortName) ? "Печать " : "";
	...
	public final String getCurrentPeriodNameText()
	{
		...
		{
		case PERIOD_COMP_RECRUITING:
			periodName = "Подготовка к новому кругу";
			break;
		case PERIOD_COMPETITION:
			periodName = "Квестовый период";
			break;
		case PERIOD_COMP_RESULTS:
			periodName = "Сбор данных евента";
			break;
		case PERIOD_SEAL_VALIDATION:
			periodName = "Период действия Печати";
			break;
		}
	...
	protected void restoreSevenSignsData()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT charId, cabal, old_cabal, seal, red_stones, green_stones, blue_stones, "
	...
	public void saveSevenSignsData(L2PcInstance player, boolean updateSettings)
	{
		...
		try
		{
			...
			for (StatsSet sevenDat : _signsPlayerData.values())
			{
				...
				PreparedStatement statement = con.prepareStatement("UPDATE seven_signs SET cabal=?, old_cabal=?, seal=?, red_stones=?, " + "green_stones=?, blue_stones=?, "
			...
			if (updateSettings)
			{
				String sqlQuery = "UPDATE seven_signs_status SET current_cycle=?, active_period=?, previous_winner=?, "
						+ "dawn_stone_score=?, dawn_festival_score=?, dawn_festival_score1=?, dawn_festival_score2=?, dawn_festival_score3=?, dawn_festival_score4=?, dawn_festival_score5=?, dusk_stone_score=?, dusk_festival_score=?, "
						+ "dusk_festival_score1=?, dusk_festival_score2=?, dusk_festival_score3=?, dusk_festival_score4=?, dusk_festival_score5=?, avarice_owner=?, gnosis_owner=?, strife_owner=?, avarice_dawn_score=?, gnosis_dawn_score=?, "
	...
	public int setPlayerInfo(L2PcInstance player, int chosenCabal, int chosenSeal)
	{
		...
		{
			// Если игрок меняет сторону (Рассвет/Закат) то его накопленный вклад
			// в древних аденах сгорает.
	...
	public void addFestivalScore(int cabal, int amount, int festivalId)
	{
		...
		{
			if (_dawnFestivalIdScore[festivalId]==0)
			{
		...
		for (Integer currSeal : _signsSealOwners.keySet())
		{
			...
			if (sealOwner != CABAL_NULL)
			{
				if (isSealValidationPeriod())
				{
					switch(currSeal)
					{
						case(SEAL_AVARICE): //Печать алчности
	...
	protected class SevenSignsPeriodChange implements Runnable
	{
		public void run()
		{
			...
			switch (periodEnded)
			{
			case PERIOD_COMP_RECRUITING: // Окончился период подготовки к новому Квестовому периоду.
				...
				break;
			case PERIOD_COMPETITION: // Действия при окончании квестового периода (7 дней)
				...
				break;
			case PERIOD_COMP_RESULTS: // Действия при окончании периода определения победителя (15 мин.)
				...
				_log.info("SevenSigns: The " + getCabalName(_previousWinner) + " have won the competition with " + getCurrentScore(_previousWinner)
						+ " points!");
				break;
			case PERIOD_SEAL_VALIDATION: // Действия при окончании периода владения Печати (7 дней)
	...
	public boolean checkSummonConditions(L2PcInstance activeChar)
	{
		...
		if (isSealValidationPeriod())
		{
			...
			{
				SystemMessage sm = new SystemMessage(SystemMessageId.SEAL_OF_STRIFE_FORBIDS_SUMMONING);
