...
import ru.catssoftware.gameserver.model.actor.instance.L2TamedBeastInstance;
...
public class FeedableBeasts extends L2AttackableAIScript
{
	...
	{
		{
			"Что вы со мной хотите сделать?", "Вы хотите приручить меня, да?",
			"Не давайте мне это. Вы можете оказатся в опасности!",
			"Ба Бах. Что это за неприятная вещь?",
			"Что это? Мы таким не питаемся! Не смей меня этим кормить!!!",
			"Что это? Могу ли я съесть это?", "Вы не должны беспокоиться обо мне.",
			"Вкусная еда, спасибо", "Я съем вашу еду", "Глоток"
		},
		{
			"Я не думаю, что Вы отказались от идеи укрощения меня.",
			"Эта еда только для меня. Может быть, я могу съесть вашу руку?",
			"Не стану ли я это этого жирным? Ха-ха", "Почему ты всегда меня кормиш?",
			"Не верьте мне. Я вас предал"
		},
		{
			"Уничтож", "Смотри, что ты сделал!",
			"Странное чувство ...! Злые намерения растут в моем сердце ...!",
			"Это происходит!", "Это печально ... Хорошо это печально ...!"
		}
	};

	private static final String[] TAMED_TEXT = 
	{
		"Стержни! Да!", "Я такой прожорливый зверь, это стыдно! Ха-ха",
		"Ваше сотрудничество чувство было все лучше и лучше.",
		"Я помогу вам!", "Погода действительно хороша. Хотите пойти на пикник?",
		"Я действительно, как вы! Это вкусно ...",
		"Если вам не придется покинуть это место, то я могу вам помочь.",
		"Чем я могу вам помочь?", "Я здесь не только для еды!",
		"Хрум, хрум, хрум, вкусно!"
	};
	...
	public FeedableBeasts ()
	{
		...
		final int[][] Kookabura_2_1 = {{21468, 21468}, {16017, 16018}};
		final int[][] Kookabura_2_2 = {{21469, 21469}, {16017, 16018}};
		...
		final int[][] Buffalo_2_1 = {{21487, 21487}, {16013, 16014}};
		final int[][] Buffalo_2_2 = {{21488, 21488}, {16013, 16014}};
		...
		final int[][] Cougar_2_1 = {{21506, 21506}, {16015, 16016}};
		final int[][] Cougar_2_2 = {{21507, 21507}, {16015, 16016}};
	...
	public void spawnNext(L2NpcInstance npc, int growthLevel, L2PcInstance player, int food)
	{
		...
		if (contains(TAMED_BEASTS, nextNpcId))
		{
			...
			int rand = Rnd.get(20);
			if (rand == 0)
				npc.broadcastPacket(new NpcSay(objectId, 0, nextNpc.getNpcId(), player.getName()+", желаете показать мне свое убежище?"));
			else if (rand == 1)
				npc.broadcastPacket(new NpcSay(objectId, 0, nextNpc.getNpcId(), player.getName()+", каждый раз, когда я смотрю на пищю, я думаю о теме!"));
			else if (rand == 2)
				npc.broadcastPacket(new NpcSay(objectId, 0, nextNpc.getNpcId(), player.getName()+", вам не стоит возвращатся в деревню, я одарю вас силой!"));
			else if (rand == 3)
				npc.broadcastPacket(new NpcSay(objectId, 0, nextNpc.getNpcId(), "Спасибо, "+player.getName()+". Я надеюсь что я смогу помочь Вам!"));
			else if (rand == 4)
				npc.broadcastPacket(new NpcSay(objectId, 0, nextNpc.getNpcId(), player.getName()+", чем я могу помочь Вам?"));
		}
		else
		{
