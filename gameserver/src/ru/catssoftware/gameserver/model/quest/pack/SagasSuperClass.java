...
public class SagasSuperClass extends Quest
{
	...
	public int[] Z = {};
	public String[] Text = {};
	FastMap<L2NpcInstance, Integer> _SpawnList = new FastMap<L2NpcInstance, Integer>();
	...
	{
		target.broadcastPacket(new MagicSkillUse(target,target,skillId,level,6000,1, false));
		target.broadcastPacket(new MagicSkillUse(npc,npc,skillId,level,6000,1, false));
	}
			...
			if (npcId == Archon_Minion)
			{
				...
				if (party != null)
				{
					...
					for (L2PcInstance player1 : party.getPartyMembers())
					{
						...
						if (st1 != null)
						{
							if (st1.getInt("cond") == 15 && st1.getPlayer().getRangeToTarget(player)<700)
