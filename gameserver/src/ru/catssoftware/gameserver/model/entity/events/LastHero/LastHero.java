...
public class LastHero extends GameEvent
{
	...
	public static final LastHero getInstance()
	{
		if(_instance==null)
			new LastHero();
		return _instance;
		...
		if(_state == GameEvent.STATE_RUNNING)
		{
			if(victim._event == this && killer._event == this && victim instanceof L2PcInstance)
			{
				if (LH_REWARD_KILLS && killer instanceof L2PcInstance)
				{
					...
					((L2PcInstance)killer).sendMessage(Message.getMessage((L2PcInstance)killer, Message.MessageId.MSG_EVENT_RECIVE_KILL_BONUS));
				}
	...
	{
		if(_state==GameEvent.STATE_RUNNING)
			return (actor._event==target._event && actor._event == this) || LH_ALLOW_INTERFERENCE;
		else
			return true;
	}
