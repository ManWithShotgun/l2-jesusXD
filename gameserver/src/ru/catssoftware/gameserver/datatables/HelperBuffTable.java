...
{
	protected static Logger			_log			= Logger.getLogger(HelperBuffTable.class.getName());
	...
	private int _magicClassLowestLevel = 101;
	private int _physicClassLowestLevel = 101;
	private int _summonClassLowestLevel = 101;
	...
	private int _summonClassHighestLevel = 1;
	...
	private void fillHelperBuffTable(ResultSet HelperBuffData) throws Exception
	{
		while (HelperBuffData.next())
		{
			...
			String ownerClass = HelperBuffData.getString("owner_class");
			if (!ownerClass.equals("Mage") && !ownerClass.equals("Fighter") && !ownerClass.equals("Summon"))
			{
				_log.warn("HelperBuffTable : owner_class field not correct fort skillID "+HelperBuffData.getInt("skill_id"));
				continue;
			}
			helperBuffDat.set("owner_class", HelperBuffData.getString("owner_class"));

			// Calulate the range level in wich player must be to obtain buff from Newbie Helper
			if ("Fighter".equals(HelperBuffData.getString("owner_class")))
			{
			...
			else if ("Mage".equals(HelperBuffData.getString("owner_class")))
			{
			...
			{
				if (HelperBuffData.getInt("lower_level") < _summonClassLowestLevel)
					_summonClassLowestLevel = HelperBuffData.getInt("lower_level");
				if (HelperBuffData.getInt("upper_level") > _summonClassHighestLevel)
					_summonClassHighestLevel = HelperBuffData.getInt("upper_level");
			}
	...
	public int getSummonClassHighestLevel()
	{
		return _summonClassHighestLevel;
	}

	public int getSummonClassLowestLevel()
	{
		return _summonClassLowestLevel;
	}
