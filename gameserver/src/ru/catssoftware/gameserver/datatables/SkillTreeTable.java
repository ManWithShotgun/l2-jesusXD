	...
	private SkillTreeTable()
	{
		...
		try
		{
			...
			while (classlist.next())
			{
				...
				PreparedStatement statement2 = con.prepareStatement("SELECT * FROM skill_trees where class_id=? ORDER BY skill_id, level");
				...
				while (skilltree.next())
				{
					...
					skillLearn = new L2SkillLearn(id, lvl, minLvl, name, cost, 0, 0,skilltree.getString("quest"));
		...
		try
		{
			...
			while (skilltree2.next())
			{
				...
				L2SkillLearn skill = new L2SkillLearn(id, lvl, minLvl, name, cost, costId, costCount,null);
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT skill_id, level, name, base_lvl, sp, min_skill_lvl, exp, success_rate76, success_rate77, success_rate78, success_rate79, success_rate80 FROM enchant_skill_trees ORDER BY skill_id, level");
			...
			while (skilltree3.next())
			{
				...
				L2EnchantSkillLearn skill = new L2EnchantSkillLearn(id, lvl, minSkillLvl, baseLvl, name, sp, exp, rate76, rate77, rate78, rate79, rate80);
		...
		for (L2SkillLearn temp : skills)
		{
			if (temp.getMinLevel() <= cha.getLevel())
			{
				...
				if (!knownSkill && temp.getLevel() == 1) {
					if(temp.getQuest()!=null) {
						QuestState qs = cha.getQuestState(temp.getQuest());
