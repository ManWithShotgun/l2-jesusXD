...
public class GameServerManager {
	...
	public boolean registerWithFirstAvailableId(GameServerInfo gsi)/*check where used*/ {
		// avoid two servers registering with the same "free" id
		/*JesusXD: haha very funny*/
		return false;
	}
	...
	private void load() {
		try {
			this.serverNamesXmlParser();
			Connection con = L2DatabaseFactory.getInstance().getConnection();
			Statement stm = con.createStatement();
			ResultSet rs = stm.executeQuery("select * from gameservers");
			while(rs.next()) {
				GameServerInfo gs = new GameServerInfo(rs);
				_gameServers.put(gs.getId(), gs);
			}
			//TODO need or not close rs, stm?
			rs.close();
			stm.close();
			con.close();
		} catch(SQLException e) {
	...
	private void serverNamesXmlParser(){
